...
--- @return boolean
local function toThrowError(expect, errorOrErrorType)
    local success, err = pcall(expect.actual)
    ...
    if errorOrErrorType ~= nil then
        if type(errorOrErrorType) == "string" then
            ...
            if not expect:checkEquals(true, message:find(errorOrErrorType) ~= nil) then
                error('Expected the function to throw an error of type ' ..
                tostring(errorOrErrorType) .. ' but it threw an error with message "' .. message .. '".')
            end
        ...
        elseif not expect:checkEquals(true, type(err) == errorOrErrorType) then
            error('Expected the function to throw an error of type ' ..
            tostring(errorOrErrorType) .. ' but it threw an error of type ' .. tostring(type(err)) .. '.')
        end
    end
