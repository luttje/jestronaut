...
local function toHaveNthReturnedWith(expect, nthCall, ...)
  ...
  if not expect:checkEquals(true, actual:hasNthReturnedWith(nthCall, ...)) then
    ...
    if tableLib.count(args) == 0 then
      error("Expected " .. tostring(actual) .. " to have returned with no arguments (on call " .. nthCall .. ") but it returned with " .. tableLib.implode(actual:getReturnedValue(nthCall), ", "))
    else
      error("Expected " .. tostring(actual) .. " to have last returned with " .. tableLib.implode(args, ", ") .. " (on call " .. nthCall .. ") but it returned with " .. tableLib.implode(actual:getReturnedValue(nthCall), ", "))
    end
  end
